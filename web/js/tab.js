tabButton = {
    ...
    init: function () {
        ...
        // переключение табов
        $('#toolbarBlock > div').on('click', function (event) {
            tabButton.clickEvent(event, this);
        });
    ...
    clickEvent: function (event, element) {
        ...
        if (undefined !== window[tabHandler])
            eval(tabHandler + '.init()');
    }
}
...
folderTab = {
    ...
    mouseOver: function (event, element) {
        if ($(element).prop('id') !== this.folderId)
            return false;
...
albumTreeTab = {
    ...
    createGalley: function (pocketNo) {
        ...
        if (!form.find('input[name="name"]').prop('value')) {
            alert('Необходимо название альбома');
            return false;
        }
        if (0 === Pocket.countImg(pocketNo)) {
            alert('Нет отмеченных изображений');
            return false;
        }
        var data = {
